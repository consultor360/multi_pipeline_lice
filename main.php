...
function multi_pipeline_activation_hook()
{
    ...
    if (validate_and_save_license()) {
        ...
        log_message('error', 'Failed to activate Multi Pipeline module: Invalid license');
    }
}
...
function multi_pipeline_check_license()
{
    $CI =& get_instance();
    $CI->load->model('multi_pipeline/license_model');
    
    $license = $CI->license_model->get_active_license();
    
    if (!$license || !$CI->license_model->validate_license($license->license_key)) {
...
// Adicione esta linha no início de cada função principal do módulo
if (!multi_pipeline_check_license()) {
    set_alert('warning', 'Multi Pipeline license is invalid or expired.');
    redirect(admin_url());
}

function create_license_table_if_not_exists()
{
    $CI =& get_instance();
    require_once(__DIR__ . '/install.php');
    create_license_table($CI);
}

function validate_and_save_license()
{
    $CI =& get_instance();
    $CI->load->model('license_model');
    $license_key = $CI->input->post('license_key');
    
    if ($CI->license_model->validate_license($license_key)) {
        $CI->license_model->set_license($license_key);
        return true;
    }
    return false;
}