...
{
    $data['title'] = _l('multi_pipeline_license');
...
{
    if (!is_admin()) {
        access_denied('Activate Multi Pipeline License');
    }

    $license_key = $this->input->post('license_key');
    $this->load->model('license_model');

    if ($this->license_model->validate_license($license_key)) {
        $this->license_model->set_license($license_key);
        set_alert('success', _l('multi_pipeline_license_activated'));
        redirect(admin_url('multi_pipeline'));
    } else {
        set_alert('danger', _l('multi_pipeline_invalid_license'));
...
public function validate_license()
{
    ...
    // Tenta criar a tabela de licença
    if ($this->license_model->create_license_table()) {
        $response = [
            'success' => true,
            'message' => _l('license_table_created_successfully')
        ];
    } else {
        $response = [
            'success' => false,
            'message' => _l('license_table_creation_failed')
        ];
    }

    // Adicione logs para depuração
    log_activity('Tentativa de criar tabela de licença: ' . json_encode($response));
